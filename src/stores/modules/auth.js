	...
	mutations: {
		setToken(state, token) {
			...
			if (token.remember) {
				localStorage.setItem(
					KEY_TOKEN,
					JSON.stringify({ token: token.token, time: new Date() })
				);
			}
		},
		setEmail(state, email) {
			...
			if (email.remember) {
				localStorage.setItem(
					KEY_EMAIL,
					JSON.stringify({ email: email.email, time: new Date() })
				);
			}
		},
		setName(state, name) {
			...
			if (name.remember) {
				localStorage.setItem(
					KEY_NAME,
					JSON.stringify({ name: name.name, time: new Date() })
				);
			}
		},
	},
	actions: {
		login({ commit, dispatch }, credentials) {
			return s_users_meetingslogin(
				...
				dispatch("users/getRegisteredUsers", null, { root: true });
			});
		},
		logout({ commit }) {
